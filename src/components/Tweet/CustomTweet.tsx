  ...
  return (
    <Card className="w-full">
      {tweet.parent && (
        <CardHeader className="pb-2">
          <div className="flex items-center space-x-2 text-sm text-gray-500">
            ...
            <Link
              href={`https://twitter.com/${tweet.parent.user.screen_name}`}
              className="text-blue-500 hover:underline"
            >
              @{tweet.parent.user.screen_name}
            </Link>
          </div>
          <p className="mt-1 text-sm text-gray-700">{tweet.parent.text}</p>
        </CardHeader>
      )}
      ...
      <CardContent>
        ...
        /> */}
        <span>{tweet.entities.map((e) => e.text).join(" ")}</span>
